...
**1. Das partes**
As partes identificadas, de um lado a **TZION TERAPIAS INTEGRATIVAS**, com CNPJ **62.422.179/0001-22**, com atendimento à Rua Princesa Isabel, Qd V Lt-01, segundo andar - Setor Santa Helena, em Araguaína TO; neste ato representada pelo terapeuta **Marcos Dany Teixeira Magalhães** com registro profissional **CRTH-BR 6793**, inscrito no CPF Nº **011.533.991-48**, e de outro lado o(a) Sr(a) **{{nome_contratante}}**, inscrito(a) no CPF sob o número **{{cpf_paciente}}**, firmam entre si o presente **TERMO DE COMPROMISSO** de Atendimento Terapêutico Integrativo, para o cliente **{{nome_paciente}}**.

**2. Atendimento**
Cada atendimento terapêutico terá a duração de 50 minutos em média, sendo realizado em horário combinado, na **TZION TERAPIAS INTEGRATIVAS**, com CNPJ **62.422.179/0001-22**, com atendimento à Rua Princesa Isabel, Qd V Lt-01, segundo andar - Setor Santa Helena, em Araguaína TO; neste ato representada pelo terapeuta: **{{nome_terapeuta}}** para a realização de terapias: **{{nome_terapia}}**, sendo o mesmo habilitado, capacitado e portador de registro profissional emitido pela ABRATH, Associação Brasileira dos Terapeutas Holísticos; estando o terapeuta à disposição do cliente no período ajustado entre as partes. Não será possível estender o horário para além do previsto, mesmo em caso de atraso do cliente.
...
**5. Dia e Horário**
Os dias e horários dos atendimentos serão acordados entre as partes. As sessões ajustadas em pacote poderão ser agendadas mensalmente; e a cada sessão para as sessões individuais.

**6. Honorários**
O pacote ajustado é de **{{quantidade_sessoes}} ({{quantidade_sessoes_extenso}}) sessões** no valor total de **{{valor_total}} ({{valor_total_extenso}})**. O pagamento será feito em dinheiro, cartão de crédito ou débito, ou transferência bancária; pagos no ato da contratação dos serviços terapêuticos.

**7. Desmarcações ou mudanças de horário**
As desmarcações deverão ser feitas com antecedência de, no mínimo, 3 horas. O terapeuta deverá ser avisado no caso de imprevistos que impeçam o comparecimento do cliente. Mudanças de horário só serão possíveis quando houver disponibilidade em agenda. As sessões não comparecidas e não justificadas em tempo hábil serão consideradas realizadas.
...
**9. Das Sessões On-Line**
As Sessões online acontecem pelas plataformas de videoconferência (ex: MEET). A secretaria fará o teste antecipado para conexão e acesso. Será respeitado o horário estabelecido para o atendimento. O equipamento deverá estar carregado e com condições de suportar a videoconferência durante toda a sessão. O cliente deverá estar em um ambiente que lhe proporcione absoluta privacidade, tranquilidade e silêncio.

**10. Transferência de Titularidade no Atendimento Terapêutico**
O atendimento terapêutico programado é pessoal e intransferível. Em hipótese alguma será permitida a transferência de um pacote pago a outra pessoa; excetua-se o caso de Terapia de Casais.

**11. O controle de Atendimento Terapêutico**
Para o atendimento presencial, o Controle de Atendimento Terapêutico, devidamente assinado, é parte integrante deste termo de compromisso. Nos atendimentos online, uma mensagem de confirmação servirá como comprovação da realização da sessão.

**12. Terapia com menores**
As terapias com menores somente serão realizadas com o devido acompanhamento dos pais ou de uma pessoa responsável, que após o atendimento receberão a devolutiva do acompanhamento ou terapia aplicados.

**13. Quanto a acompanhamento psiquiátrico e medicação**
...
**15. Das devoluções de valores**
Em casos de insatisfação quanto às consultas e sessões e possíveis devoluções de valores, fica estabelecido que será necessária manifestação das partes até a **SEGUNDA SESSÃO TERAPÊUTICA**, descontados os respectivos custos operacionais referentes a despesas operacionais, taxa da máquina de cartão de crédito e/ou débito, locação de salas e demais despesas adicionais.

**16. De demandas e desacordos**
As partes aqui qualificadas, de comum acordo, decidem desde agora que toda e qualquer controvérsia que venha a existir em relação à execução do presente Termo de Compromisso será decidida pelo Sistema de Arbitragem nos termos da Lei Federal 13.129/15.

E por estarem justas e contratadas as partes, assinam o presente instrumento particular.
...
export function fillContractTemplate(template: string, data: {
  ...
  // Remove qualquer resquício antigo de tags de RG se o template customizado do banco ainda contiver
  result = result
    .replace(/com\s+Identidade\s+n[º°]\s*\{\{rg_paciente\}\}\s*expedido\s+por\s*\{\{orgao_emissor_rg\}\}\s*na\s+data\s*\{\{data_emissao_rg\}\}\s*e\s+CPF/gi, 'inscrito(a) no CPF')
    .replace(/com\s+Identidade\s+n[º°]\s*\{\{rg\}\}\s*expedido\s+por\s*\{\{orgao_emissor_rg\}\}\s*na\s+data\s*\{\{data_emissao_rg\}\}\s*e\s+CPF/gi, 'inscrito(a) no CPF');
