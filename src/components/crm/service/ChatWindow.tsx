...
export default function ChatWindow({ ticket, onBack }: Props) {
  ...
  const cancelRecording = () => {
    if (mediaRecorderRef.current && mediaRecorderRef.current.state === 'recording') {
      mediaRecorderRef.current.onstop = null; // Evita que o evento de envio seja disparado
      mediaRecorderRef.current.stop();
      mediaRecorderRef.current.stream.getTracks().forEach(track => track.stop());
    }
            ...
            {isRecording ? (
              <div className="flex-1 flex items-center justify-between py-2 text-rose-500 font-bold animate-pulse">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={stopRecording} className="p-2 bg-[#00a884] hover:bg-[#008f6f] rounded-full text-white transition-colors" title="Enviar áudio">
